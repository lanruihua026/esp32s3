...
// ===== 称重参数 =====
// calibration_factor2：每 1 克对应的原始 ADC 差值（raw / g），通过 calibrateScale2() 校准后更新
 ...
 * 公式：calibration_factor2 = 净原始值 / 实际重量
 */
void calibrateScale2(float knownWeight)
{
    ...
    float netValue = currentValue - zero_offset2;        // 减去零点得到净原始值

    // calibration_factor2 = 净原始值 / 实际重量（克）
 ...
 *
 * 说明：
 * 1. 读取当前空载平均值作为 zero_offset2，后续重量计算基于此零点。
 * 2. 重置方向锁定标志，允许重新识别受力方向，适应接线或放置方式调整。
 * 3. 在 setupHX711_2() 完成初始去皮；运行时也可手动调用，适应环境变化。
 */
void tareScale2()
{
    if (!isScaleReady2)
    ...
    zero_offset2 = readAverageRaw2(samples);  // 将当前空载均值记为零点偏移
